...
├── config/
│   ├── industries.json            # 产业分类参考配置
│   └── sources.json               # RSS源参考配置
├── src/
│   ├── main.py                    # 主程序入口
│   ├── config.py                  # 实际运行的数据源和产业配置
│   ├── fetcher.py                 # 采集调度器
│   ├── fetchers/                  # RSS/API/网页/PDF/社交媒体采集插件
│   ├── processor.py               # AI分析器
...
- **自动采集**：从 RSS、NewsAPI、宏观数据 API、网页、PDF 和社交媒体源抓取信息
- **AI分析**：自动分类产业、提取实体、判断影响、标记风险
- **本地兜底分析**：未配置 AI 密钥或 AI 调用失败时，自动使用关键词规则生成可读日报
- **结构化指标**：世界银行、央行汇率等指标按最新数据入报，不消耗AI额度
...
| Secret名称 | 说明 | 获取方式 |
|-----------|------|---------|
| `DEEPSEEK_API_KEY` | DeepSeek API密钥（推荐，优先使用） | [platform.deepseek.com](https://platform.deepseek.com) |
| `OPENAI_API_KEY` | OpenAI API密钥（未配置 DeepSeek 时使用） | [platform.openai.com](https://platform.openai.com) |
| `WECHAT_WEBHOOK_URL` | 企业微信机器人Webhook（可选） | 企业微信群 → 添加机器人 → 复制Webhook地址 |
| `NEWSAPI_KEY` | NewsAPI 新闻源密钥（可选） | [newsapi.org](https://newsapi.org) |
| `TWITTER_BEARER_TOKEN` | X/Twitter 抓取密钥（可选） | X Developer Platform |
| `TELEGRAM_API_ID` / `TELEGRAM_API_HASH` | Telegram 抓取密钥（可选） | [my.telegram.org](https://my.telegram.org) |
| `TRADING_ECONOMICS_KEY` | Trading Economics 密钥（可选） | Trading Economics |
...
如果本地网络无法访问新闻源，仍可使用验收样本模式检查完整生成链路：

```bash
python -m src.main --demo
```

该模式不访问网络，不代表真实日报内容，只用于确认采集后处理、分析、落盘、HTML 生成和微信跳过逻辑都能跑通。
...
### 修改RSS源
编辑 `src/config.py` 中的 `RSS_SOURCES` 或其他数据源列表。`config/sources.json` 目前仅作为参考配置。

### 修改产业分类
编辑 `src/config.py` 中的 `SECTORS` 和 `SECTOR_KEYWORDS`。`config/industries.json` 目前仅作为参考配置。

### 调整采集数量
修改 `src/fetcher.py` 中的 `max_items` 参数（默认80条）。

### 调整宏观指标
编辑 `src/config.py` 中的 `API_SOURCES`。世界银行指标默认只取每个指标的最新非空年度值，避免历史数据每天重复进入日报。
...
**Q: 采集不到新闻？**
A: 部分源可能失效，检查 `src/fetcher.py` 运行日志，在 `src/config.py` 中更新源地址。
...
- feedparser + BeautifulSoup + Playwright + pdfplumber（多源采集）
- 纯HTML/CSS（移动端适配）
